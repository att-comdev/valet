...
from valet import storage
from valet.tests import base as test_base
try:
    from valet.tests import mocks
except ImportError:
...
class PgSQLManager(SQLManager):
    ...
    def __init__(self, url):
        self._url = url
        self._db_name = 'valet_%s' % uuid.uuid4().hex
...
class ElasticSearchManager(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        self.connection = storage.get_connection(
            'sqlite://', 'valet.metering.storage')
        self.alarm_connection = storage.get_connection(
            'sqlite://', 'valet.alarm.storage')
        self.event_connection = storage.get_connection(
            self.url, 'valet.event.storage')
...
class HBaseManager(fixtures.Fixture):
    ...
    def url(self):
        return '%s?table_prefix=%s' % (
            self._url,
            os.getenv("VALET_TEST_HBASE_TABLE_PREFIX", "test")
        )
...
class TestBase(testscenarios.testcase.WithScenarios, test_base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.CONF([], project='valet', validate_default_values=True)
        ...
        self.useFixture(mockpatch.Patch('valet.storage.get_connection',
                                        side_effect=self._get_connection))

        # Set a default location for the pipeline config file so the
        # tests work even if valet is not installed globally on
        # the system.
        self.CONF.import_opt('pipeline_cfg_file', 'valet.pipeline')
        self.CONF.set_override(
            'pipeline_cfg_file',
            self.path_get('etc/valet/pipeline.yaml')
        )
    ...
    def _get_connection(self, url, namespace):
        if namespace == "valet.alarm.storage":
            return self.alarm_conn
        elif namespace == "valet.event.storage":
...
class MixinTestsWithBackendScenarios(object):
    ...
    for db in ('MONGODB', 'MYSQL', 'PGSQL', 'HBASE', 'DB2', 'ES'):
        if os.environ.get('VALET_TEST_%s_URL' % db):
            scenarios.append(
                (db.lower(), {'db_url': os.environ.get(
                    'VALET_TEST_%s_URL' % db)}))
    ...
    if 'mongodb' in scenarios_db and 'db2' not in scenarios_db:
        scenarios.append(
            ('db2', {'db_url': os.environ.get('VALET_TEST_MONGODB_URL',
                                              '').replace('mongodb://',
                                                          'db2://')}))
