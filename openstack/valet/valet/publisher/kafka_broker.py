...
from valet.i18n import _LE
from valet.publisher import messaging
...
class KafkaBrokerPublisher(messaging.MessagingPublisher):
    ...
    valet pipeline configuration file. If an ip address is not specified,
    this kafka publisher will not publish any meters.

    To enable this publisher, add the following section to the
    /etc/valet/pipeline.yaml file or simply add it to an existing
    pipeline::
    ...
    metering data under a topic name, 'valet'. If the port number is not
    ...
    def __init__(self, parsed_url):
        ...
        self._topic = options.get('topic', ['valet'])[-1]
