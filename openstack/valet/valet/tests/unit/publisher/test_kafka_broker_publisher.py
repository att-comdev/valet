...
# under the License.
"""Tests for valet/publisher/kafka_broker.py
"""
import datetime
import uuid
...
from valet.event.storage import models as event
from valet.publisher import kafka_broker as kafka
from valet.publisher import messaging as msg_publisher
from valet import sample
from valet.tests import base as tests_base


@mock.patch('valet.publisher.kafka_broker.LOG', mock.Mock())
@mock.patch('valet.publisher.kafka_broker.kafka.KafkaClient',
            mock.Mock())
class TestKafkaPublisher(tests_base.BaseTestCase):
    ...
    def test_publish(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=valet'))
    ...
    def test_publish_to_host_without_policy(self):
        ...
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=valet&policy=test'))
        self.assertEqual('default', publisher.policy)

    def test_publish_to_host_with_default_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=valet&policy=default'))
    ...
    def test_publish_to_host_with_drop_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=valet&policy=drop'))
    ...
    def test_publish_to_host_with_queue_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=valet&policy=queue'))
    ...
    def test_publish_event_with_default_policy(self):
        publisher = kafka.KafkaBrokerPublisher(
            netutils.urlsplit('kafka://127.0.0.1:9092?topic=valet'))
