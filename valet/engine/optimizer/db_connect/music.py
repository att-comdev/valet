...
class REST(object):
    ...
    def request(self, method='get', content_type='application/json',
        ...
        for url in self.urls:
            ...
            try:
                response = method_fn(url + path, data=json.dumps(data), headers=self.__headers(content_type), timeout=self.timeout)
...
class Music(object):
    ...
    def create_table(self, keyspace, table, schema):
        ...
        path = '/keyspaces/%(keyspace)s/tables/%(table)s/' % {'keyspace': keyspace, 'table': table}
        response = self.rest.request(method='post', path=path, data=data)
        return response.ok
    ...
    def create_row(self, keyspace, table, values):
        ...
        path = '/keyspaces/%(keyspace)s/tables/%(table)s/rows' % {'keyspace': keyspace, 'table': table}
        response = self.rest.request(method='post', path=path, data=data)
        return response.ok
    ...
    def update_row_atomically(self, keyspace, table,
        ...
        # TODO(GY): Wouldn't we delete the lock at this point?
    ...
    # TODO(GY): Shouldn't this really be part of internal cleanup?
