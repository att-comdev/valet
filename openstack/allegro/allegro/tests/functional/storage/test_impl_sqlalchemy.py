...
# under the License.
"""Tests for allegro/storage/impl_sqlalchemy.py

.. note::
  In order to run the tests against real SQL server set the environment
  variable ALLEGRO_TEST_SQL_URL to point to a SQL server before running
  the tests.
...
from allegro.alarm.storage import impl_sqlalchemy as impl_sqla_alarm
from allegro.event.storage import impl_sqlalchemy as impl_sqla_event
from allegro.event.storage import models
from allegro.storage import impl_sqlalchemy
from allegro.storage.sqlalchemy import models as sql_models
from allegro.tests import base as test_base
from allegro.tests import db as tests_db
from allegro.tests.functional.storage \
    import test_storage_scenarios as scenarios


@tests_db.run_with('sqlite', 'mysql', 'pgsql')
class AllegroBaseTest(tests_db.TestBase):

    def test_allegro_base(self):
        base = sql_models.AllegroBase()
        base['key'] = 'value'
