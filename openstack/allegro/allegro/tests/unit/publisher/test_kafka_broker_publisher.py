...
# under the License.
"""Tests for allegro/publisher/kafka_broker.py
"""
import datetime
import uuid
...
from allegro.event.storage import models as event
from allegro.publisher import kafka_broker as kafka
from allegro.publisher import messaging as msg_publisher
from allegro import sample
from allegro.tests import base as tests_base


@mock.patch('allegro.publisher.kafka_broker.LOG', mock.Mock())
@mock.patch('allegro.publisher.kafka_broker.kafka.KafkaClient',
            mock.Mock())
class TestKafkaPublisher(tests_base.BaseTestCase):
    ...
    def test_publish(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=allegro'))
    ...
    def test_publish_to_host_without_policy(self):
        ...
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=allegro&policy=test'))
        self.assertEqual('default', publisher.policy)

    def test_publish_to_host_with_default_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=allegro&policy=default'))
    ...
    def test_publish_to_host_with_drop_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=allegro&policy=drop'))
    ...
    def test_publish_to_host_with_queue_policy(self):
        publisher = kafka.KafkaBrokerPublisher(netutils.urlsplit(
            'kafka://127.0.0.1:9092?topic=allegro&policy=queue'))
    ...
    def test_publish_event_with_default_policy(self):
        publisher = kafka.KafkaBrokerPublisher(
            netutils.urlsplit('kafka://127.0.0.1:9092?topic=allegro'))
