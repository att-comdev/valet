...
class Analyzer(object):
    ...
    def get_host_name(self, instance_name, attr):
        '''
    ...
    def are_different(self, hosts_list, level):
        ...
        try:
            for h in hosts_list:
                if self.is_already_exists(diction, self.get_host_or_rack(level, h)) == True:
                    return False
            return True
    ...
    def are_the_same(self, hosts_list, level):
        try:
            for h in hosts_list:
                if self.compare_host(self.get_host_or_rack(level, h), self.get_host_or_rack(level, hosts_list[0])) == False:
                    return False
            return True
    ...
    def is_already_exists(self, diction, item):
        if item in diction: return True
