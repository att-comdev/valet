...
class BinSendSampleTestCase(base.BaseTestCase):
    def setUp(self):
        ...
        pipeline_cfg_file = self.path_get('etc/valet/pipeline.yaml')
        content = ("[DEFAULT]\n"
                   "rpc_backend=fake\n"
    ...
    def test_send_counter_run(self):
        subp = subprocess.Popen(['valet-send-sample',
                                 "--config-file=%s" % self.tempfile,
...
class BinApiTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        content = ("[pipeline:main]\n"
                   "pipeline = api-server\n"
                   "[app:api-server]\n"
                   "paste.app_factory = valet.api.app:app_factory\n")
        if six.PY3:
            content = content.encode('utf-8')
        ...
        # create valet.conf file
        self.api_port = random.randint(10000, 11000)
        self.http = httplib2.Http(proxy_info=None)
        self.pipeline_cfg_file = self.path_get('etc/valet/pipeline.yaml')
        self.policy_file = self.path_get('etc/valet/policy.json')
    ...
    def run_api(self, content, err_pipe=None):
        ...
        self.tempfile = fileutils.write_to_tempfile(content=content,
                                                    ...
                                                    suffix='.conf')
        if err_pipe:
            return subprocess.Popen(['valet-api',
                                    "--config-file=%s" % self.tempfile],
                                    stderr=subprocess.PIPE)
        else:
            return subprocess.Popen(['valet-api',
...
class BinValetPollingServiceTestCase(base.BaseTestCase):
    def setUp(self):
        super(BinValetPollingServiceTestCase, self).setUp()
        content = ("[DEFAULT]\n"
                   "rpc_backend=fake\n"
                   "[database]\n"
                   "connection=log://localhost\n")
    ...
    def tearDown(self):
        super(BinValetPollingServiceTestCase, self).tearDown()
        if self.subp:
            self.subp.kill()
        os.remove(self.tempfile)

    def test_starting_with_duplication_namespaces(self):
        self.subp = subprocess.Popen(['valet-polling',
                                      "--config-file=%s" % self.tempfile,
                                      "--polling-namespaces",
                                      "compute",
                                      "compute"],
