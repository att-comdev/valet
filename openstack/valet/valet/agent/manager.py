...
from valet.agent import plugin_base
from valet import coordination
from valet.i18n import _, _LI, _LE, _LW
from valet import keystone_client
from valet import messaging
from valet import pipeline
from valet.publisher import utils as publisher_utils
from valet import service_base
...
cfg.CONF.import_opt('telemetry_driver', 'valet.publisher.messaging',
                    group='publisher_notifier')
cfg.CONF.import_group('service_types', 'valet.energy.kwapi')
cfg.CONF.import_group('service_types', 'valet.image.glance')
cfg.CONF.import_group('service_types', 'valet.neutron_client')
cfg.CONF.import_group('service_types', 'valet.nova_client')
cfg.CONF.import_group('service_types', 'valet.objectstore.rgw')
cfg.CONF.import_group('service_types', 'valet.objectstore.swift')
...
class AgentManager(service_base.BaseService):
    ...
    def _extensions(self, category, agent_ns=None):
        namespace = ('valet.%s.%s' % (category, agent_ns) if agent_ns
                     else 'valet.%s' % category)
        return self._get_ext_mgr(namespace)

    def _extensions_from_builder(self, category, agent_ns=None):
        ns = ('valet.builder.%s.%s' % (category, agent_ns) if agent_ns
              else 'valet.builder.%s' % category)
