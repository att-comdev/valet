...
class FilePublisher(publisher.PublisherBase):
    ...
    location should be configured in allegro pipeline configuration file.
    If a file name and location is not specified, this File Publisher will not
    log any meters other than log a warning in Allegro log file.

    To enable this publisher, add the following section to the
    /etc/allegro/publisher.yaml file or simply add it to an existing
    pipeline::
