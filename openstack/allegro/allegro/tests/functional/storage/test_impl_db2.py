...
# under the License.
"""Tests for allegro/storage/impl_db2.py

.. note::
  In order to run the tests against another MongoDB server set the
  environment variable ALLEGRO_TEST_DB2_URL to point to a DB2
...
from allegro.alarm.storage import impl_db2 as impl_db2_alarm
from allegro.event.storage import impl_db2 as impl_db2_event
from allegro.storage import impl_db2
from allegro.storage.mongo import utils as pymongo_utils
...
class ConnectionTest(test_base.BaseTestCase):
    ...
    def test_upgrade(self, meter_id, timestamp, mongo_connect,
        ...
        conn_mock.allegdb2.resource.index_information.return_value = {}
        ...
        cfg.CONF.set_override('db2nosql_resource_id_maxlen',
                              256,
                              group='database')
        impl_db2.Connection('db2://user:pwd@localhost:27017/allegdb2')
        resource_id = 'wew' * 247 + 'x' * 3
        conn_mock.allegdb2.resource.insert_one.assert_called_with(
            {'_id': resource_id,
             'no_key': resource_id})
        conn_mock.allegdb2.meter.insert_one.assert_called_with(
    ...
    def test_generate_random_str_with_less_config_len(self, objectid,
        ...
        cfg.CONF.set_override('db2nosql_resource_id_maxlen',
                              20,
                              group='database')
        conn = impl_db2.Connection('db2://user:pwd@localhost:27017/allegdb2')
