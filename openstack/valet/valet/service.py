...
CLI_OPTS = [
    cfg.StrOpt('os-username',
               ...
               default=os.environ.get('OS_USERNAME', 'valet'),
...
def prepare_service(argv=None, config_files=None):
    ...
    if argv is None:
        argv = sys.argv
    cfg.CONF(argv[1:], project='valet', validate_default_values=True,
             version=version.version_info.version_string(),
             default_config_files=config_files)
    log.setup(cfg.CONF, 'valet')
    # NOTE(liusheng): guru cannot run with service under apache daemon, so when
    # valet-api running with mod_wsgi, the argv is [], we don't start
    # guru.
    if argv:
