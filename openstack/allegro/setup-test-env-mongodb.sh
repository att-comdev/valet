...
MONGO_DATA=`mktemp -d /tmp/ALLEG-MONGODB-XXXXX`
MONGO_PORT=29000
...
export ALLEGRO_TEST_MONGODB_URL="mongodb://localhost:${MONGO_PORT}/allegro"
if test -n "$ALLEGRO_TEST_HBASE_URL"
then
    export ALLEGRO_TEST_HBASE_TABLE_PREFIX=$(hexdump -n 16 -v -e '/1 "%02X"' /dev/urandom)
