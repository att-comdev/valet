...
from allegro.agent import plugin_base
from allegro import coordination
from allegro.i18n import _, _LI, _LE, _LW
from allegro import keystone_client
from allegro import messaging
from allegro import pipeline
from allegro.publisher import utils as publisher_utils
from allegro import service_base
from allegro import utils
...
cfg.CONF.import_opt('telemetry_driver', 'allegro.publisher.messaging',
                    group='publisher_notifier')
cfg.CONF.import_group('service_types', 'allegro.energy.kwapi')
cfg.CONF.import_group('service_types', 'allegro.image.glance')
cfg.CONF.import_group('service_types', 'allegro.neutron_client')
cfg.CONF.import_group('service_types', 'allegro.nova_client')
cfg.CONF.import_group('service_types', 'allegro.objectstore.rgw')
cfg.CONF.import_group('service_types', 'allegro.objectstore.swift')
...
class AgentManager(service_base.BaseService):
    ...
    def _extensions(self, category, agent_ns=None):
        namespace = ('allegro.%s.%s' % (category, agent_ns) if agent_ns
                     else 'allegro.%s' % category)
        return self._get_ext_mgr(namespace)

    def _extensions_from_builder(self, category, agent_ns=None):
        ns = ('allegro.builder.%s.%s' % (category, agent_ns) if agent_ns
              else 'allegro.builder.%s' % category)
