...
from valet.alarm.evaluator import gnocchi
...
class TestGnocchiThresholdEvaluate(base.TestEvaluatorBase):
    ...
    def setUp(self):
        ...
        self.useFixture(mockpatch.Patch('valetclient.client.get_client',
                                        return_value=self.api_client))
        self.requests = self.useFixture(mockpatch.Patch(
            'valet.alarm.evaluator.gnocchi.requests')).mock
    ...
    @unittest.skipIf(six.PY3,
                     "the valet base class is not python 3 ready")
    @mock.patch.object(timeutils, 'utcnow')
