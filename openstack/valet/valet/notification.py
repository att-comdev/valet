...
from valet.agent import plugin_base as base
from valet import coordination
from valet.event import endpoint as event_endpoint
from valet.i18n import _, _LI, _LW
from valet import exchange_control
...
OPTS = [
    ...
    cfg.BoolOpt('disable_non_metric_meters',
                default=True,
                help='WARNING: Valet historically offered the ability to '
...
class NotificationService(service_base.BaseService):
    ...
    NOTIFICATION_NAMESPACE = 'valet.notification'
    NOTIFICATION_IPC = 'valet-pipe'
    ...
    def _get_notifiers(self, transport, pipe):
        ...
        for x in range(cfg.CONF.notification.pipeline_processing_queues):
            notifiers.append(oslo_messaging.Notifier(
                ...
                publisher_id='valet.notification',
    ...
    def start(self):
        ...
        if cfg.CONF.notification.workload_partitioning:
            ...
            # not a valet. Until we have something to get the
        ...
        if not cfg.CONF.notification.disable_non_metric_meters:
            LOG.warning(_LW('Non-metric meters may be collected. It is highly '
                            ...
                            'valet.conf or the pipeline.yaml'))
