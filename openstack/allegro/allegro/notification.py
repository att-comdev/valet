...
from allegro.agent import plugin_base as base
from allegro import coordination
from allegro.event import endpoint as event_endpoint
from allegro.i18n import _, _LI, _LW
from allegro import exchange_control
...
OPTS = [
    ...
    cfg.BoolOpt('disable_non_metric_meters',
                default=True,
                help='WARNING: Allegro historically offered the ability to '
...
class NotificationService(service_base.BaseService):
    ...
    NOTIFICATION_NAMESPACE = 'allegro.notification'
    NOTIFICATION_IPC = 'allegro-pipe'
    ...
    def _get_notifiers(self, transport, pipe):
        ...
        for x in range(cfg.CONF.notification.pipeline_processing_queues):
            notifiers.append(oslo_messaging.Notifier(
                ...
                publisher_id='allegro.notification',
    ...
    def start(self):
        ...
        if cfg.CONF.notification.workload_partitioning:
            ...
            # not a allegro. Until we have something to get the
        ...
        if not cfg.CONF.notification.disable_non_metric_meters:
            LOG.warning(_LW('Non-metric meters may be collected. It is highly '
                            ...
                            'allegro.conf or the pipeline.yaml'))
