...
from allegro.i18n import _LE
from allegro.publisher import messaging
...
class KafkaBrokerPublisher(messaging.MessagingPublisher):
    ...
    allegro pipeline configuration file. If an ip address is not specified,
    this kafka publisher will not publish any meters.

    To enable this publisher, add the following section to the
    /etc/allegro/pipeline.yaml file or simply add it to an existing
    pipeline::
    ...
    metering data under a topic name, 'allegro'. If the port number is not
    ...
    def __init__(self, parsed_url):
        ...
        self._topic = options.get('topic', ['allegro'])[-1]
