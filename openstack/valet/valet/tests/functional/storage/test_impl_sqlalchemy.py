...
# under the License.
"""Tests for valet/storage/impl_sqlalchemy.py

.. note::
  In order to run the tests against real SQL server set the environment
  variable VALET_TEST_SQL_URL to point to a SQL server before running
  the tests.
...
from valet.alarm.storage import impl_sqlalchemy as impl_sqla_alarm
from valet.event.storage import impl_sqlalchemy as impl_sqla_event
from valet.event.storage import models
from valet.storage import impl_sqlalchemy
from valet.storage.sqlalchemy import models as sql_models
from valet.tests import base as test_base
from valet.tests import db as tests_db
from valet.tests.functional.storage \
    import test_storage_scenarios as scenarios


@tests_db.run_with('sqlite', 'mysql', 'pgsql')
class ValetBaseTest(tests_db.TestBase):

    def test_valet_base(self):
        base = sql_models.ValetBase()
        base['key'] = 'value'
