...
from allegro.publisher import utils
...
class ConfigFixture(fixture.GabbiFixture):
    ...
    def start_fixture(self):
        ...
        for engine in ENGINES:
            try:
                db_url = os.environ['ALLEGRO_TEST_%s_URL' % engine]
            except KeyError:
                pass
        if db_url is None:
        ...
        conf.import_group('api', 'allegro.api.controllers.v2.root')
        conf.import_opt('store_events', 'allegro.notification',
                        group='notification')
        conf.set_override('policy_file',
                          os.path.abspath('etc/allegro/policy.json'),
                          group='oslo_policy')

        # A special pipeline is required to use the direct publisher.
        conf.set_override('pipeline_cfg_file',
                          'etc/allegro/gabbi_pipeline.yaml')
