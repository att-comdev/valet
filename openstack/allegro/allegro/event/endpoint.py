...
from allegro.event import converter as event_converter
...
class EventsNotificationEndpoint(object):
    def __init__(self, manager):
        ...
        self.event_converter = event_converter.setup_events(
            extension.ExtensionManager(
                namespace='allegro.event.trait_plugin'))
        self.manager = manager

    def info(self, ctxt, publisher_id, event_type, payload, metadata):
        """Convert message at info level to Allegro Event.
    ...
    def error(self, ctxt, publisher_id, event_type, payload, metadata):
        """Convert error message to Allegro Event.
