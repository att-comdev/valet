...
from valet.engine.optimizer.ostro.search_base \
    import Node, Resource, LogicalGroupResource
from valet.engine.optimizer.ostro.search_base \
    import SwitchResource, StorageResource
...
class Search(object):
    ...
    def re_place_nodes(self, _app_topology, _resource):
        ...
        if len(self.app_topology.old_vm_map) > 0:
            ...
            self.logger.debug("Search: adjust resources by deducting prior "
                              "placements")
    ...
    def _place_planned_nodes(self):
        init_level = LEVELS[len(LEVELS) - 1]
        (planned_node_list, level) = \
            self._open_planned_list(self.app_topology.vms,
                                    self.app_topology.volumes,
                                    self.app_topology.vgroups,
                                    init_level)
    ...
    def _get_best_resource_for_planned(self, _n, _level, _avail_resources):
        ...
        if _level == "host" and (isinstance(_n.node, VM) or isinstance(_n.node,
                                                                       Volume)):
            ...
            (planned_node_list, level) = self._open_planned_list(vms, volumes,
                                                                 vgroups,
                                                                 _level)
            ...
            if isinstance(rack, Datacenter):
                ...
                for rsk in rack.storages.keys():
                    if rsk in self.avail_storage_hosts.keys():
                        r.rack_avail_storages[rsk] = \
                            self.avail_storage_hosts[rsk]
                ...
                if isinstance(cluster, Datacenter):
                    ...
                    for csk in cluster.storages.keys():
                        if csk in self.avail_storage_hosts.keys():
                            r.cluster_avail_storages[csk] = \
                                self.avail_storage_hosts[csk]

                    for csk in cluster.switches.keys():
                        if csk in self.avail_switches.keys():
                            r.cluster_avail_switches[csk] = \
                                self.avail_switches[csk]
    ...
    def _compute_resource_weights(self):
        ...
        for (r, w) in self.app_topology.optimization_priority:
            if r == "bw":
                self.logger.debug("    nw weight = " +
                                  str(self.nw_bandwidth_weight))
            elif r == "cpu":
                self.logger.debug("    cpu weight = " + str(self.CPU_weight))
            elif r == "mem":
                self.logger.debug("    mem weight = " + str(self.mem_weight))
            elif r == "lvol":
                self.logger.debug("    local disk weight = " +
                                  str(self.local_disk_weight))
            elif r == "vol":
    ...
    def _get_best_resource(self, _n, _level, _avail_resources):
        ...
        if target == "bw":
            ...
            if len(candidate_list) == 0:
                self.status = "no available network bandwidth left, for " \
                              "node = " + _n.node.name
                self.logger.error("Search: " + self.status)
                return None
            ...
            if target == "vol":
                ...
                if isinstance(_n.node, VGroup) or isinstance(_n.node, VM):
                    ...
                    self._set_disk_sort_base(_level, candidate_list,
                                             _n.node.volume_class)
                    candidate_list.sort(key=operator.attrgetter("sort_base"),
                                        reverse=True)

            top_candidate_list = self._sort_lowest_bandwidth_usage(
                _n, _level, candidate_list)
        ...
        if _level == "host" and (isinstance(_n.node, VM) or isinstance(_n.node,
            ...
            while True:
                ...
                while len(top_candidate_list) > 0:
                    ...
                    self.logger.debug("Search: try candidate = " +
                                      cr.get_resource_name(_level))
                    ...
                    (open_node_list, level) = self._open_list(vms, volumes,
                                                              vgroups, _level)
                    if open_node_list is None:
                        break
                    ...
                    if self._run_greedy(open_node_list, level, avail_hosts) \
                        ...
                        self.logger.debug("Search: rollback of candidate "
                ...
                if best_resource is not None:
                    ...
                    if len(candidate_list) == 0:
                        ...
                        if target == "bw":
                            top_candidate_list = \
                                self._sort_highest_consolidation(_n, _level,
                                                                 candidate_list)
                        else:
                            top_candidate_list = \
                                self._sort_lowest_bandwidth_usage(
                                    _n, _level, candidate_list)
                            if len(top_candidate_list) == 0:
                                self.status = "no available network " \
                                              "bandwidth left"
                                self.logger.warn("Search: " + self.status)
                                break
                ...
                if isinstance(_n.node, Volume):
                    ...
                    for vck in _n.node.volume_sizes.keys():
                        if _n.node.volume_sizes[vck] > max_size:
                            max_size = _n.node.volume_sizes[vck]
                            volume_class = vck
                self._set_disk_sort_base(_level, top_candidate_list,
                                         volume_class)
                top_candidate_list.sort(key=operator.attrgetter("sort_base"),
                                        reverse=True)
            else:
    ...
    def _estimate_max_bandwidth(self, _level, _n, _candidate):
        nw_bandwidth_penalty = self._estimate_nw_bandwidth_penalty(_level, _n,
                                                                   _candidate)
    ...
    def _estimate_nw_bandwidth_penalty(self, _level, _n, _candidate):
        ...
        # 3rd entry to be used for special node communicating beyond datacenter
        # or zone
                ...
                if v.uuid == vl.node.uuid:
                    ...
                    if placement_level != "ANY" and \
                        ...
                        sort_base += compute_reservation(_level,
                                                         placement_level,
                                                         bandwidth)
                        self.constraint_solver.get_req_bandwidths(
                            _level, placement_level, bandwidth, req_bandwidths)

        candidate = copy.deepcopy(_candidate)

        exclusivity_ids = self.constraint_solver.get_exclusivities(
        ...
        if exclusivity_id is not None:
            ...
            self._add_exclusivity_to_candidate(_level, candidate,
                                               exclusivity_id)
        ...
        self._deduct_reservation_from_candidate(candidate, _n, req_bandwidths,
                                                _level)
        ...
        for vl in link_list:
            ...
            if diversity_level == "ANY":
                implicit_diversity = \
                    self.constraint_solver.get_implicit_diversity(_n.node,
                                                                  link_list,
                                                                  vl.node,
                                                                  _level)
            ...
            if diversity_level == "ANY" or \
                ...
                if vg.uuid not in handled_vgroups.keys():
                    ...
                    self.constraint_solver.get_req_bandwidths(
                        _level, _level, bandwidth, temp_req_bandwidths)

                    if self._check_availability(_level, temp_n, candidate) \
                            is True:
                        self._deduct_reservation_from_candidate(
                            candidate, temp_n, temp_req_bandwidths, _level)
                    else:
                        sort_base += compute_reservation(_level, _level,
                                                         bandwidth)
                ...
                self.constraint_solver.get_req_bandwidths(_level,
                                                          ...
                                                          bandwidth,
                                                          req_bandwidths)
                sort_base += compute_reservation(_level, diversity_level,
                                                 bandwidth)

        if self.constraint_solver._check_nw_bandwidth_availability(
                _level, req_bandwidths, _candidate) is False:
            sort_base = -1
    ...
    def _add_exclusivity_to_candidate(self, _level, _candidate,
                                      _exclusivity_id):
    ...
    def _check_availability(self, _level, _n, _candidate):
        if isinstance(_n.node, VM):
            if self.constraint_solver.check_cpu_capacity(_level, _n.node,
                                                         _candidate) is False:
                return False
            if self.constraint_solver.check_mem_capacity(_level, _n.node,
                                                         _candidate) is False:
                return False
            if self.constraint_solver.check_local_disk_capacity(
                    _level, _n.node, _candidate) is False:
                return False
        elif isinstance(_n.node, Volume):
            if self.constraint_solver.check_storage_availability(
                    _level, _n.node, _candidate) is False:
                return False
        else:
            if self.constraint_solver.check_cpu_capacity(
                    _level, _n.node, _candidate) is False or \
               self.constraint_solver.check_mem_capacity(
                   _level, _n.node, _candidate) is False or \
               self.constraint_solver.check_local_disk_capacity(
                   _level, _n.node, _candidate) is False or \
               self.constraint_solver.check_storage_availability(
                   _level, _n.node, _candidate) is False:
                return False

        if self.constraint_solver.check_nw_bandwidth_availability(
                _level, _n, self.node_placements, _candidate) is False:
            return False

        if isinstance(_n.node, VM):
            if len(_n.node.extra_specs_list) > 0:
                if self.constraint_solver.check_host_aggregates(
                        _level, _candidate, _n.node) is False:
                    return False

        if isinstance(_n.node, VM):
            if _n.node.availability_zone is not None:
                if self.constraint_solver.check_availability_zone(
                        _level, _candidate, _n.node) is False:
                    return False

        if self.constraint_solver.conflict_diversity(
                _level, _n, self.node_placements, _candidate) is True:
            return False
        ...
        if len(exclusivities) == 1:
            ...
            if self.constraint_solver.exist_group(
                    _level, exc_id, "EX", _candidate) is False:
                return False
        ...
        if aff_id is not None:
            if aff_id in self.avail_logical_groups.keys():
                if self.constraint_solver.exist_group(
                        _level, aff_id, "AFF", _candidate) is False:
                    return False
    ...
    def _deduct_reservation_from_candidate(self, _candidate, _n, _rsrv, _level):
        ...
        if isinstance(_n.node, Volume) or isinstance(_n.node, VGroup):
            self._deduct_candidate_volume_reservation(_level, _n.node,
                                                      _candidate)
    ...
    def _deduct_candidate_nw_reservation(self, _candidate, _rsrv):
        ...
        for srk in _candidate.cluster_avail_switches.keys():
            ...
            if sr.switch_type == "spine":
                sr.avail_bandwidths = [bw - _rsrv[2] for bw
    ...
    def _add_exclusivity(self, _level, _best, _exclusivity_id):
        ...
        if _exclusivity_id not in self.avail_logical_groups.keys():
            ...
            self.logger.debug("Search: add new exclusivity (" +
                              _exclusivity_id + ")")
        else:
            lgr = self.avail_logical_groups[_exclusivity_id]
    ...
    def _add_diversities(self, _level, _best, _diversity_id):
        ...
        if _diversity_id not in self.avail_logical_groups.keys():
            ...
            self.logger.debug("Search: add new diversity (" +
                              _diversity_id + ")")
        else:
    ...
    def _deduct_vm_resources(self, _best, _n):
        ...
        for vml in _n.node.vm_list:
            if vml.node in self.node_placements.keys():
                ...
                self.bandwidth_usage += self._deduct_nw_reservation(
                    placement_level, chosen_host, cn, bandwidth)
    ...
    def _deduct_nw_reservation(self, _placement_level, _host1, _host2, _rsrv):
        ...
        if _placement_level == "host":
            ...
            for _, sr in _host2.cluster_avail_switches.iteritems():
                if sr.switch_type == "spine":
                    sr.avail_bandwidths = [bw - _rsrv for bw
                                           in sr.avail_bandwidths]
    ...
    def _rollback_vm_reservation(self, _v):
        if _v in self.node_placements.keys():
            ...
            for vml in _v.vm_list:
                if vml.node in self.node_placements.keys():
                    cn = self.avail_hosts[
                        self.node_placements[vml.node].host_name
                    ]
                    level = cn.get_common_placement(chosen_host)
                    bandwidth = vml.nw_bandwidth
                    self.bandwidth_usage -= self._rollback_nw_reservation(
                        level, chosen_host, cn, bandwidth)

            for voll in _v.volume_list:
                if voll.node in self.node_placements.keys():
                    cn = self.avail_hosts[
                        self.node_placements[voll.node].host_name
                    ]
                    level = cn.get_common_placement(chosen_host)
                    bandwidth = voll.io_bandwidth
                    self.bandwidth_usage -= self._rollback_nw_reservation(
