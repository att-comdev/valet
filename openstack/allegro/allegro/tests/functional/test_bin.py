...
from allegro.tests import base
...
class BinTestCase(base.BaseTestCase):
    ...
    def test_dbsync_run(self):
        subp = subprocess.Popen(['allegro-dbsync',
                                 "--config-file=%s" % self.tempfile])
        self.assertEqual(0, subp.wait())

    def test_run_expirer_ttl_disabled(self):
        subp = subprocess.Popen(['allegro-expirer',
                                 '-d',
                                 "--config-file=%s" % self.tempfile],
...
class BinSendSampleTestCase(base.BaseTestCase):
    def setUp(self):
        ...
        pipeline_cfg_file = self.path_get('etc/allegro/pipeline.yaml')
        content = ("[DEFAULT]\n"
                   "rpc_backend=fake\n"
    ...
    def test_send_counter_run(self):
        subp = subprocess.Popen(['allegro-send-sample',
                                 "--config-file=%s" % self.tempfile,
...
class BinApiTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        content = ("[pipeline:main]\n"
                   "pipeline = api-server\n"
                   "[app:api-server]\n"
                   "paste.app_factory = allegro.api.app:app_factory\n")
        if six.PY3:
            content = content.encode('utf-8')
        ...
        # create allegro.conf file
        self.api_port = random.randint(10000, 11000)
        self.http = httplib2.Http(proxy_info=None)
        self.pipeline_cfg_file = self.path_get('etc/allegro/pipeline.yaml')
        self.policy_file = self.path_get('etc/allegro/policy.json')
    ...
    def run_api(self, content, err_pipe=None):
        ...
        self.tempfile = fileutils.write_to_tempfile(content=content,
                                                    ...
                                                    suffix='.conf')
        if err_pipe:
            return subprocess.Popen(['allegro-api',
                                    "--config-file=%s" % self.tempfile],
                                    stderr=subprocess.PIPE)
        else:
            return subprocess.Popen(['allegro-api',
...
class BinAllegroPollingServiceTestCase(base.BaseTestCase):
    def setUp(self):
        super(BinAllegroPollingServiceTestCase, self).setUp()
        content = ("[DEFAULT]\n"
                   "rpc_backend=fake\n"
                   "[database]\n"
                   "connection=log://localhost\n")
    ...
    def tearDown(self):
        super(BinAllegroPollingServiceTestCase, self).tearDown()
        if self.subp:
            self.subp.kill()
        os.remove(self.tempfile)

    def test_starting_with_duplication_namespaces(self):
        self.subp = subprocess.Popen(['allegro-polling',
                                      "--config-file=%s" % self.tempfile,
                                      "--polling-namespaces",
                                      "compute",
                                      "compute"],
